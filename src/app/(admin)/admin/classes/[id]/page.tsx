...
import {
  PageHeader, Card, Field, Input, Select, Button,
...
export default async function ManageClassPage({
  ...
  return (
    <div className="space-y-8">
      <PageHeader title="Manage class" description={classRow.name} />
      ...
      {/* Schedule */}
      <section>
        <h2 className="mb-3 font-semibold text-slate-900">Weekly schedule</h2>
        {schedules && schedules.length > 0 ? (
          ...
          <EmptyState message="No schedule slots yet." />
        )}

        <Card className="mt-4 max-w-3xl p-6">
          <form action={addSchedule} className="grid items-end gap-4 sm:grid-cols-5">
            <input type="hidden" name="class_id" value={classRow.id} />
            <Field label="Day">
      ...
      {/* Coaches */}
      <section>
        <h2 className="mb-3 font-semibold text-slate-900">Coaches</h2>
        ...
        <Card className="mt-4 max-w-md p-6">
          <form action={addCoach} className="flex items-end gap-3">
            <input type="hidden" name="class_id" value={classRow.id} />
      ...
      {/* Enrollment */}
      <section>
        <h2 className="mb-3 font-semibold text-slate-900">Enrolled students</h2>
        ...
        <Card className="mt-4 max-w-md p-6">
          <form action={enrollStudent} className="flex items-end gap-3">
            <input type="hidden" name="class_id" value={classRow.id} />
            <div className="flex-1">
      ...
      {/* Sessions */}
      <section>
        <h2 className="mb-3 font-semibold text-slate-900">Sessions</h2>
        <Card className="flex items-center justify-between p-6">
          <div className="text-sm text-slate-600">
            <span className="text-2xl font-semibold text-slate-900">{sessionCount ?? 0}</span> session(s) scheduled.
          </div>
